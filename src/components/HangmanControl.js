      ...
      <div className="main">
        {!this.props.game ? (
          ...
          <div className="game-screen">
            ...
            <h1 className="word">
              {this.props.wordToGuess.map((letter, i) => (
                <span key={i}>{letter.guessed ? letter.letter : "_"} </span>
              ))}
            </h1>
            <div className="button-cont">
              {alph.map((letter) => (
                <button
                  key={letter}
                  onClick={(event) => this.guess(letter, event)}
                >
                  {letter}
                </button>
              ))}
            ...
            <div className="lives">
              {this.props.lives > 0 ? (
                <Rate
                  ...
                  style={{ color: "red" }}
                  value={this.props.lives}
                />
              ) : (
                <p>
                  {" "}
                  You lost :( the word was <strong>{this.displayWord()}</strong>
                </p>
              )}
            </div>
